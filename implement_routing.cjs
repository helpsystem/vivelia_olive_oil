...
// 1. Setup Views Container
html = html.replace(
  '<div class="flex flex-col w-full text-on-surface">',
  '<div id="view-home" class="flex flex-col w-full text-on-surface transition-opacity duration-300">'
);
html = html.replace(
  '</main>',
  '</div>\n  <div id="view-product" class="hidden opacity-0 flex-col w-full text-on-surface min-h-screen bg-surface relative z-50 transition-opacity duration-300"></div>\n</main>'
);

// 2. Remove the old Modal HTML entirely
html = html.replace(
  /<!-- Product Details Modal \(Bottom Sheet\) -->[\s\S]*?<\/div>\s*<\/div>/,
  '<!-- Views injected above -->'
);

// 3. Replace openProductModal and closeProductModal with openProductPage and closeProductPage
const scriptRegex = /window\.openProductModal\s*=\s*function\(id\)\s*\{[\s\S]*?window\.closeProductModal\s*=\s*function\(\)\s*\{[\s\S]*?\}\s*;/;
...
const newScript = `
    ...
    window.openProductPage = function(id) {
      ...
      if (p.actionType === 'order') {
        ctaHtml = \`<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8">
          <button onclick="closeProductPage(); setTimeout(()=>openZelleCheckout('\${p.orderPack || 'duo'}', '\${p.zelleTitle || title}', \${p.zellePrice || p.price}), 100);" class="py-4 px-6 bg-[#D4AF37] hover:bg-[#e4c256] text-[#0D1B2A] rounded-2xl font-label-caps text-sm font-extrabold uppercase transition-all shadow-md flex items-center justify-center gap-2 active:scale-95" type="button">
            <span class="material-symbols-outlined text-[20px]">bolt</span>
            <span>Zelle ($ \${p.zellePrice || p.price})</span>
          </button>
          <button onclick="closeProductPage(); setTimeout(()=>preselectAndOrderProduct('\${p.id}', '\${p.orderPack || 'duo'}'), 100);" class="py-4 px-6 bg-[#0D1B2A] hover:bg-[#1b2a3a] text-white rounded-2xl font-label-caps text-sm font-bold uppercase transition-all shadow-md flex items-center justify-center gap-2 active:scale-95" type="button">
            <span class="material-symbols-outlined text-[20px] text-[#D4AF37]">shopping_cart_checkout</span>
            <span>\${isEs ? 'Ordenar Ahora' : 'Order Now'}</span>
          </button>
        </div>\`;
      } else if (p.actionType === 'whatsapp_notify') {
        ctaHtml = \`<div class="mt-8">
          <a href="\${p.whatsappUrl}" target="_blank" rel="noopener noreferrer" class="w-full py-4 px-6 bg-[#25D366] hover:bg-[#20b858] text-white rounded-2xl font-label-caps text-sm font-bold uppercase transition-all shadow-md flex items-center justify-center gap-2 active:scale-95">
...
// 4. Update the sticky card onclick to openProductPage
html = html.replace(/onclick="openProductModal\(/g, 'onclick="openProductPage(');
